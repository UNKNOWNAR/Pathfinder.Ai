...
We have fully migrated to **PostgreSQL with the `pgvector` extension**. We are no longer using ChromaDB. All job and student semantic vectors are stored directly alongside their relational data in the `pathfinder` database using Amazon RDS, simplifying our infrastructure.

---

## Architecture Overview
1. **Database:** Amazon RDS (PostgreSQL 15+ with `pgvector` enabled)
2. **Backend Engine:** Amazon EC2 (Ubuntu 22.04 LTS) running Gunicorn + Nginx
3. **Frontend Application:** AWS Amplify (Vue.js static hosting)

---

## 1. Provisioning the Database (AWS RDS)
*This handles our users, jobs, relational data, and semantic embeddings.*

1. Log into the AWS Management Console and navigate to **RDS**.
2. Click **Create database**.
3. Choose **Standard create** and select **PostgreSQL** (version 15 or higher).
...
8. **Connectivity:**
   - Public access: **Yes** (to allow your local machine to run migrations initially, or **No** if you prefer to only connect via EC2).
   - Create a new VPC security group: `pathfinder-db-sg`.
9. Click **Create database**.
10. **Post-Creation:** Connect to the database using an SQL client (like DataGrip, DBeaver or `psql`) and run this required command to enable vector matching:
    ```sql
    CREATE EXTENSION IF NOT EXISTS vector;
    ```
11. Update your EC2 `.env` file with this connection string:
    `SQLALCHEMY_DATABASE_URI="postgresql://postgres:<PASSWORD>@<ENDPOINT>:5432/postgres"`

---

## 2. Deploying the Backend (AWS EC2)
*This hosts our Python Flask API and Harvester Engine.*
...
4. **Instance Type:** `t3.small` or `t3.medium`.
   *(Note: sentence-transformers require around 1GB-2GB of RAM to load the ML models into memory. A `.micro` instance may crash).*
...
7. **Storage:** 16 GiB is sufficient.
...
server {
    ...
    location / {
        ...
        # Handle CORS locally in Nginx if needed, though Flask-CORS should handle it
    }
}
```
Enable the site:
...
4. Authorize AWS and select the `Pathfinder.Ai` repository and your working branch (`master`).
...
3. Check the EC2 logs (`sudo journalctl -u pathfinder -f`) to ensure successful vector matching with pgvector via Flask-SQLAlchemy.
